...
class HealthChecker:
    ...
    def _auth_headers(self) -> dict[str, str]:
        token = os.environ.get("OPENSPACE_LOCAL_SERVER_TOKEN", "").strip()
    ...
    def check_gui_execute_endpoint(self) -> Tuple[bool, str]:
        """Functionally test the GUI-only execute endpoint"""
        try:
                ...
                json={
                    "command": [
                        ...
                        (
                            ...
                            "pyautogui.FAILSAFE = False; print('hello_test')"
                        ),
                    ],
                    "shell": False,
                },
            ...
            if 'hello_test' in output:
                return True, "GUI execute endpoint works"
    ...
    def check_accessibility(self) -> Tuple[bool, str]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/accessibility?max_depth=1",
                timeout=10,
                headers=self._auth_headers(),
            )
    ...
    def check_all(self, test_endpoints: bool = True) -> Dict[str, HealthStatus]:
        ...
        test_functions = {
            ...
            'GUI Execute': self.check_gui_execute_endpoint,
